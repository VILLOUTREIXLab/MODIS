...
def flatten_omegaconf(config, parent_key: str = '') -> dict:
    """Flatten a nested OmegaConf object into a single-level dictionary.

    Nested keys are represented using dot notation
    (e.g., ``'parent.child.key'``).

    Args:
        config (omegaconf.DictConfig or omegaconf.ListConfig): The OmegaConf
            object to flatten.
        parent_key (str): Base key prefix for the current recursion level.
            Defaults to ``''``.
        ...
        if not config.modalities or len(config.modalities) < 2:
            ...
            for i, modality in enumerate(config.modalities):
                if 'name' not in modality:
                    ...
                    error = (
                        f'Invalid input_size for modality {modality.name}: '
                        f'{modality.input_size}. Must be positive integer.'
                    )
                    break
                    ...
                    if not isinstance(modality.encoder_ratios, (list, ListConfig)):
                        ...
                        error = (
                            f'Invalid encoder_ratios for modality {modality.name}: '
